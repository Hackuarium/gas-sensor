...
/*****************************  getAnalogReadH2 *********************************************3
Input:   /
Output:  PARAM_H2 set as the ppm of H2, wth errors
Remarks: TODO
************************************************************************************/ 
float getAnalogReadH2(){
  int i;
  float sensorValue = 0;
  
  for(i = 0; i < SAMPLE_NUM; i++){
    sensorValue += analogRead(H2_PIN);
    delay(SAMPLE_INTERVAL);
  }
  sensorValue = sensorValue/SAMPLE_NUM;
  
  if(sensorValue <= 10){
    setParameter(PARAM_H2, ERROR_MIN);
  }
  else if(sensorValue >= 1023){
    setParameter(PARAM_H2, ERROR_MAX);
  }
  else{
  getH2(sensorValue);
  }
...
Remarks: The sensor and the load resistor forms a voltage divider. Given the voltage
         ...
         could be derived.
************************************************************************************/ 
int MQResistanceCalculation(int raw_adc){
...
Remarks: By using the slope and a point of the line. The x(logarithmic value of ppm) 
         ...
         value.
************************************************************************************/ 
int  MQGetPercentageH2(float rs_ro_ratio, float *pcurve){
